...
$('assign-form').addEventListener('submit', async (e) => {
  ...
  await createNotification(assigned_to, 'task_assigned', 'New task assigned', title, inserted.id);
...
function renderWorkInbox() {
  ...
  renderMyTickets(buckets[inboxTab] || []);
}
...
function renderMyTickets(tasks) {
  ...
  tasks.forEach(t => {
    ...
    card.innerHTML = `
      ...
      <div class="meta ${isOverdue(t) ? 'overdue' : ''}">Deadline: ${formatDate(t.deadline)}</div>
  ...
  grid.querySelectorAll('button[data-accept]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (t) await notifyBoss('task_accepted', `${currentProfile.full_name} accepted a task`, t.title, taskId);
      await loadEmployeeData();
    });
  });
  ...
  grid.querySelectorAll('button[data-start]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const t = allTasksCache.find(x => x.id === taskId);
      if (t) await notifyBoss('task_started', `${currentProfile.full_name} started work`, t.title, taskId);
      await loadEmployeeData();
    });
  });
  ...
  grid.querySelectorAll('button[data-complete]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const t = allTasksCache.find(x => x.id === taskId);
      if (t) await notifyBoss('task_completed', `${currentProfile.full_name} completed a task`, t.title, taskId);
      await loadEmployeeData();
    });
  });
  ...
  for (const tmpl of templates) {
    ...
    while (next.getTime() <= today.getTime() && safety < 60) {
      ...
      if (!insertErr && inserted) {
        await createNotification(tmpl.assigned_to, 'task_assigned', 'New task assigned', tmpl.title, inserted.id);
        await sb.from('task_events').insert({
          task_id: inserted.id,
          event_type: 'assigned',
          from_employee: null,
...
// =========================================================
// NOTIFICATION CENTER (bell icon, real-time)
// =========================================================
...
async function checkDeadlineNotifications() {
  ...
  for (const t of myOpenTasks) {
    if (t.deadline === tomorrow) {
      ...
      if (!exists) await createNotification(currentUser.id, 'deadline_tomorrow', 'Deadline tomorrow', t.title, t.id);
    } else if (isOverdue(t)) {
      const exists = notificationsCache.some(n => n.task_id === t.id && n.type === 'overdue');
      if (!exists) await createNotification(currentUser.id, 'overdue', 'Task overdue', t.title, t.id);
    }
  }
}
